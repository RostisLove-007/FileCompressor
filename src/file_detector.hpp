...
// ═══════════════════════════════════════════════════════════════
//  Автоматическое определение типа файла по магическим байтам
// ═══════════════════════════════════════════════════════════════
class FileDetector {
    struct Magic {
        std::vector<uint8_t> bytes;
        size_t               offset;   // смещение в файле
        std::string          mime;
    ...
    static const std::vector<Magic>& signatures() {
        static std::vector<Magic> sigs = {
            ...
            // DOCX/XLSX/PPTX – это ZIP с content-type внутри
...
public:
    ...
    static Result detect(const std::vector<uint8_t>& data) {
        ...
        // Fallback: текстовый или бинарный
