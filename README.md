# Руководство пользователя — CZip v2.0
...
13. [Проверка целостности CRC32](#13-проверка-целостности-crc32)
14. [Чанковая обработка больших файлов](#14-чанковая-обработка-больших-файлов)
15. [Формат архивного файла](#15-формат-архивного-файла)
16. [Сообщения об ошибках](#16-сообщения-об-ошибках)
...
![Секция «Формат архива» — переключатели CZIP и CRAR](docs/screenshots/5.png)
...
![Секция «Метод сжатия» — переключатели «Huffman» и «LZ77 + Huffman»](docs/screenshots/6.png)
...
![Секция «Размер окна LZ77» с числовым полем и выбранным значением](docs/screenshots/7.png)
...
![Главное окно в режиме «Восстановить» — скрытые секции формата и метода](docs/screenshots/8.png)
...
> **⚠ Важно:** флажок «Продолжить» имеет смысл только при наличии `.ckpt`-файла от предыдущего незавершённого сжатия того же файла.

![Флажок «Продолжить прерванное сжатие» в активном состоянии и статусная строка «Продолжение с чанка N из M»](docs/screenshots/9.png)
...
1. В строке статуса появится сообщение: _«Отмена... (будет сохранена контрольная точка)»_.
...
Благодаря контрольной точке работу можно продолжить позже без потери уже сжатых данных (см. раздел 10).

![Кнопка «Отмена» в нажатом состоянии и статусная строка «Отмена...»](docs/screenshots/10.png)
...
- **Текстовая метка статуса** — показывает текущую операцию, например:
  - _«Сжатие чанка 3 из 12...»_
  - _«Продолжение с чанка 5 из 12...»_
  - _«Отмена... (будет сохранена контрольная точка)»_
  - _«⚠ Файл повреждён — восстановление прервано»_
...
![Полоса прогресса на ~60% с текстом «Сжатие чанка 7 из 12...»](docs/screenshots/11.png)

---

## 13. Проверка целостности CRC32

При **распаковке** программа автоматически проверяет контрольную сумму CRC32, сохранённую в заголовке архива:

- Если CRC32 совпадает — данные не повреждены, операция завершается успешно.
- Если CRC32 не совпадает — в строке статуса появляется предупреждение _«⚠ Файл повреждён — восстановление прервано»_ и показывается диалог ошибки.

При **сжатии** CRC32 накапливается инкрементально (по чанкам) и записывается в заголовок архива по завершении операции, что не требует дополнительного прохода по файлу.

![Диалог ошибки с сообщением о повреждённом архиве (CRC32 mismatch)](docs/screenshots/12.png)

---

## 14. Чанковая обработка больших файлов

Программа разделяет исходный файл на чанки по **512 КБ** и обрабатывает их последовательно. Это позволяет:

- Работать с файлами любого размера без загрузки всего содержимого в оперативную память.
- Корректно отображать прогресс операции чанк за чанком.
- Сохранять контрольные точки после каждого чанка (для возобновления).

**Адаптивное сохранение:** если сжатый чанк оказывается крупнее исходного (что возможно для уже сжатых данных — JPEG, MP4, ZIP), чанк записывается в архив **без сжатия** с соответствующим флагом. Это защищает от ситуации, когда «сжатый» файл больше оригинала.

![Строка статуса «Сжатие чанка 1 из N» для большого файла (>100 МБ)](docs/screenshots/13.png)

---

## 15. Формат архивного файла

Оба формата (CZIP и CRAR) используют единую структуру заголовка:

```
Смещение  Размер   Поле
────────────────────────────────────────────
0         4 байта  Магическое число: "CZIP" или "CRAR"
4         1 байт   Версия формата (0x01)
5         1 байт   Метод сжатия
6         1 байт   Флаги
7         4 байта  Размер окна win_size (big-endian)
11        8 байт   Исходный размер orig_size (big-endian)
19        4 байта  CRC32 оригинальных данных (big-endian)
23        2 байта  Длина имени файла
25        N байт   Имя исходного файла
25+N      4 байта  Количество чанков n_chunks
...       —        Чанки: [4B сжатый размер][4B исходный размер][данные]
```

![Hex-дамп начала .czip-файла с выделенными полями заголовка](docs/screenshots/14.png)
...
![Модальное диалоговое окно с сообщением об ошибке](docs/screenshots/15.png)
...
Для текстовых файлов и исходного кода — LZ77 + Huffman даёт лучшую степень сжатия. Для однократного быстрого архивирования — Huffman. Для CRAR-формата алгоритм выбирается автоматически.

**Можно ли распаковать архив без указания метода?**  
Да. При распаковке программа читает метод из заголовка архива — вручную ничего выбирать не нужно.

**Что происходит с `.ckpt`-файлом при успешном завершении?**  
Контрольная точка удаляется автоматически. Если файл остался — операция была прервана и её можно возобновить.

**Поддерживает ли программа сжатие нескольких файлов или папок?**  
Нет, текущая версия (v2.0) работает с одним файлом за раз.

**Что означает «адаптивное сохранение чанка»?**  
Если сжатый чанк получился крупнее исходного (например, для JPEG), он сохраняется без сжатия. Итоговый архив никогда не будет значительно больше оригинала из-за этой оптимизации.

![Финальный диалог «Операция успешно завершена!» после сжатия](docs/screenshots/16.png)

---

*Руководство составлено на основе анализа исходного кода CZip v2.0. Скриншоты необходимо добавить после сборки и запуска приложения.*
